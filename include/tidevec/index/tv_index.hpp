...
public:
    ...
    std::vector<SearchResult> search(const std::vector<float>& query,
        ...
        for (int layer = top_layer; layer >= 1; --layer) {
            ep = _greedy_closest(query, ep, layer, qt, m);
        }
        ...
        for (int i = 0; i < k; ++i) {
            ...
            float raw_sim = compute_similarity(query, node.embedding, m);
...
private:
    ...
    void _insert_locked(const CortexVector& vec) {
        ...
        for (int layer = top_layer; layer > new_level; --layer)
            ep = _greedy_closest(nodes_[new_id].embedding, ep, layer, qt, m);
        ...
        for (int layer = std::min(new_level, top_layer); layer >= 0; --layer) {
            ...
            auto cands = _beam_search(nodes_[new_id].embedding, ep, layer,
            ...
            for (int i = 0; i < take; ++i) {
                ...
                _prune_neighbours(nb, layer, M_layer, qt, m);
            }
    ...
    uint64_t _greedy_closest(const std::vector<float>& query,
                              ...
                              Timestamp qt, Metric m) const {
        uint64_t cur = start;
        float cur_score = _blended_score(query, cur, qt, m);
        bool improved = true;
        while (improved) {
            improved = false;
            for (uint64_t nb : nodes_[cur].neighbours[layer]) {
                if (deleted_.count(nb)) continue;
                float s = _blended_score(query, nb, qt, m);
    ...
    _beam_search(const std::vector<float>& query,
        ...
        float ep_score = _blended_score(query, ep, qt, m);
        W.push({ep_score, ep});
        C.push({ep_score, ep});
        visited.insert(ep);
    ...
    float _blended_score(const std::vector<float>& query,
                         uint64_t iid, Timestamp qt, Metric m) const {
        const auto& node = nodes_[iid];
        float sim = compute_similarity(query, node.embedding, m);
    ...
    void _prune_neighbours(uint64_t node_id, int layer, int M_max,
        ...
        // Keep M_max closest by blended score
        const auto& emb = nodes_[node_id].embedding;
        std::sort(nbs.begin(), nbs.end(), [&](uint64_t a, uint64_t b){
            return _blended_score(emb, a, qt, m) >
                   _blended_score(emb, b, qt, m);
        });
        nbs.resize(M_max);
    }
